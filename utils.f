...
: SET_BSC1					( value starting_bit_position bits_num -- )
	BSC1_BASE ROT ROT MASK_REGISTER 	\ We define a general word for any field of the BSC1_BASE register
	OR BSC1_BASE ! ;			\ We then define more specific words for ease of use

: SET_IN_9					( -- )
	0 9 SET_GPFSEL ;			\ We set GPIO 9 to INPUT
	
: SET_IN_10					( -- )
	0 10 SET_GPFSEL ;			\ We set GPIO 10 to INPUT
...
: CLEAR_FIFO					( -- )
	20 4 2 SET_BSC1 ;			\ It clears the FIFO

: SET_I2CEN					( -- )
	8000 F 1 SET_BSC1 ;			\ It enables the BSC controller
		
: START_TRANSFER				( -- )
	80 7 1 SET_BSC1	;			\ It starts a new transfer
